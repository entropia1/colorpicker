 ...
 * <P>
 * However this panel is also resizable, and it can exist in other contexts. For
 * example, you might try the following panel: <BR>
 ...
 * values of a color. The <code>main()</code> method of this class demonstrates
...
public class ColorPicker extends JPanel {
	...
	protected static ResourceBundle strings = ResourceBundle
			.getBundle("com.bric.swing.ColorPicker");
	...
	public static void main(String[] args) {
		...
		final ColorPicker picker = new ColorPicker(true, false, false, null);
		...
		picker.addPropertyChangeListener(MODE_PROPERTY,
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ColorPicker.newOverlay(null, color, modeCheckbox.isSelected(),
						alphaCheckbox.isSelected(), null).setVisible(true);
				if (color != null)
	 ...
	 */
	public static ColorPickerOverlay newOverlay(String title,
			Color originalColor, boolean showExpertControls,
			boolean includeOpacity, ChangeListener externalChangeListener) {
		return newOverlay(title, originalColor, showExpertControls,
				includeOpacity, false, externalChangeListener);
	}
	 ...
	 *         the user cancels the dialog.
	 */
	private static ColorPickerOverlay newOverlay(String title,
			Color originalColor, boolean showExpertControls,
			boolean includeOpacity, boolean hideColorPickerPanel,
			ChangeListener externalChangeListener) {
		ColorPickerOverlay d;
		d = new ColorPickerOverlay(originalColor, showExpertControls,
				includeOpacity, hideColorPickerPanel, externalChangeListener);
	...
	/**
	 * This creates an overlay which lets the user choose a gray value.
	 * 
	 * @param owner
	 *            the dialog this new dialog belongs to. This must be a Frame or
	 *            a Dialog. Java 1.6 supports Windows here, but this package is
	 ...
	 *            component is a <code>Window</code>.
	 * @param title
	 *            the title for the dialog or null to use the default title.
	 * @param initialGrayValue
	 *            the gray value the <code>ColorPicker</code> initially points
	 *            to.
	 * @param includeOpacity
	 *            whether to add a control for the opacity of the color.
	 * @return the <code>Color</code> the user chooses, or <code>null</code> if
	 *         the user cancels the dialog.
	 */
	public static ColorPickerOverlay newGrayValueOverlay(String title,
			int initialGrayValue, boolean includeOpacity,
			ChangeListener externalChangeListener) {
		ColorPickerOverlay newOverlay = newOverlay(title, new Color(
				initialGrayValue, initialGrayValue, initialGrayValue), true,
				includeOpacity, true, externalChangeListener);
		configureGrayValueColorPicker(newOverlay.getColorPicker());
		newOverlay.pack();
		return newOverlay;
	}

	private static void configureGrayValueColorPicker(ColorPicker colorPicker) {
		colorPicker.setModeControlsVisible(false);
		colorPicker.setHexControlsVisible(false);
		colorPicker.setRGBControlsVisible(false);
		colorPicker.setHueControlVisible(false);
		colorPicker.setSaturationControlVisible(false);
	}
	 ...
	 *         the user cancels the dialog.
	 */
	private static ColorPickerDialog newDialog(String title,
			Color originalColor, boolean showExpertControls,
			boolean includeOpacity, boolean hideColorPickerPanel) {
		ColorPickerDialog d;
		d = new ColorPickerDialog(originalColor, showExpertControls,
				includeOpacity, hideColorPickerPanel);
	 ...
	 *         the user cancels the dialog.
	 */
	public static ColorPickerDialog newDialog(String title,
			Color originalColor, boolean showExpertControls,
			boolean includeOpacity) {
		return newDialog(title, originalColor, showExpertControls,
	 ...
	 *         the user cancels the dialog.
	 */
	public static ColorPickerDialog newGrayValueDialog(String title,
			int initialGrayValue, boolean includeOpacity) {
		ColorPickerDialog newDialog = newDialog(title, new Color(
				initialGrayValue, initialGrayValue, initialGrayValue), true,
				includeOpacity, true);
		configureGrayValueColorPicker(newDialog.getColorPicker());
		newDialog.pack();
		return newDialog;
	}
	...
	ChangeListener externalChangeListener = null;
	...
	/**
	 * This thread will wait a second or two before committing the text in the
	 * hex TextField. This gives the user a chance to finish typing... but if
	 * the user is just waiting for something to happen, this makes sure after a
		...
		public void run() {
			if (SwingUtilities.isEventDispatchThread() == false) {
				...
				while (System.currentTimeMillis() - myStamp < WAIT) {
					try {
						long delay = WAIT
								- (System.currentTimeMillis() - myStamp);
						if (delay < 1)
							delay = 1;
						Thread.sleep(delay);
					} catch (Exception e) {
		...
		public void changedUpdate(DocumentEvent e) {
			...
			if (s.length() == 6) {
				...
				try {
					...
					// this shouldn't happen, since we already stripped out
					// non-hex characters.
					e2.printStackTrace();
				}
			}
	...
	public ColorPicker(boolean showExpertControls, boolean includeOpacity,
			boolean hideColorPanel, ChangeListener externalChangeListener) {
		 ...
		 * 
		 * @Override public void mousePressed(MouseEvent e) { focusHexField(); }
		 * }); hexField.addFocusListener(new FocusAdapter() {
		 * 
		 * @Override public void focusLost(FocusEvent e) { hexFieldUnfocused();
		 * } }); hexField.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { hexFieldUnfocused(); } });
		 */
		setMode(BRI);
		...
		setColorPanelVisible(!hideColorPanel);
	 ...
	 */
	public int[] getRGB() {
		return new int[] { red.getIntValue(), green.getIntValue(),
				blue.getIntValue() };
	}
	...
	public void setOpacity(float v) {
		...
		try {
			...
			if (lastOpacity != v) {
				firePropertyChange(OPACITY_PROPERTY, new Float(lastOpacity),
						new Float(i));
	...
	/**
	 * This shows or hides the hue control of the HSB spinner controls.
	 * <P>
	 * Note these live inside the "expert controls", so if
	 * <code>setExpertControlsVisible(false)</code> has been called, then
	 * calling this method makes no difference: the HSB controls will be hidden.
	 ...
	 *            whether the controls should be visible or not.
	 */
	public void setHueControlVisible(boolean b) {
		hue.setVisible(b);
	}

	/**
	 * This shows or hides the saturation control of the HSB spinner controls.
	 * <P>
	 * Note these live inside the "expert controls", so if
	 * <code>setExpertControlsVisible(false)</code> has been called, then
	 * calling this method makes no difference: the HSB controls will be hidden.
	 ...
	 *            whether the controls should be visible or not.
	 */
	public void setSaturationControlVisible(boolean b) {
		sat.setVisible(b);
	}

	/**
	 * This shows or hides the brightness control of the HSB spinner controls.
	 * <P>
	 * Note these live inside the "expert controls", so if
	 * <code>setExpertControlsVisible(false)</code> has been called, then
	 * calling this method makes no difference: the HSB controls will be hidden.
	 ...
	 *            whether the controls should be visible or not.
	 */
	public void setBrightnessControlVisible(boolean b) {
		bri.setVisible(b);
	}
	...
	public void setColorPanelVisible(boolean b) {
	...
	public synchronized void addKeyListener(KeyListener l) {
		alpha.addKeyListener(l);
		hue.addKeyListener(l);
		bri.addKeyListener(l);
		sat.addKeyListener(l);
		red.addKeyListener(l);
		green.addKeyListener(l);
		blue.addKeyListener(l);
		preview.addKeyListener(l);
		hexField.addKeyListener(l);
	}

	@Override
	public synchronized void removeKeyListener(KeyListener l) {
		alpha.removeKeyListener(l);
		blue.removeKeyListener(l);
		bri.removeKeyListener(l);
		sat.removeKeyListener(l);
		red.removeKeyListener(l);
		green.removeKeyListener(l);
		blue.removeKeyListener(l);
		preview.removeKeyListener(l);
		hexField.removeKeyListener(l);
	}
